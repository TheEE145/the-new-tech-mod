...
import the.mod.content.Blocksx;
import the.mod.content.Liquids;
...
public class RedconPlanetGenerator extends PlanetGenerator{
    ...
    public static final Block
            sand     = Blocksx.mantium,
            darkSand = Blocksx.virusMFloor,
            ice      = Blocksx.orangeIce,
            stone    = Blocksx.mantium,
            water2   = Blocksx.emethen,
            basalt   = Blocksx.crystals,
            salt     = Blocksx.crystals,
            snow     = Blocksx.crystals;
    ...
    protected void generate(){
        ...
        for(int i = 0; i < tlen; i++){
            ...
            if(tile.block() == air){
                total ++;
                if(tile.floor().liquidDrop == Liquids.emethen) {
                    waters ++;
                }
            }
        }
        ...
        state.rules.spawns = Waves.generate(difficulty,
                new Rand(sector.id), state.rules.attackMode, state.rules.attackMode && spawner.countGroundSpawns() == 0, naval);
        ...
        for(Tile t : tiles) {
            if((t.block() == Blocks.coreShard || t.block() == Blocks.coreFoundation || t.block() == Blocks.coreNucleus) && t.build != null) {
                ...
                if(Mathf.chance(0.0025f) && t.block() == air) {
                    t.setNet(Blocksx.virusMCrystal);
                }
            }
        }
    }
